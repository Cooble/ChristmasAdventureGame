...
public final class MPlayer2 {
    ...
    public static void loopSound(String soundName, double volume) {
        ...
        loopSound.loop(Game.getSettings().soundVolume);
    }
    ...
    public static void playSound(@NotNull String soundName, double volume, int delayTicks) {
        ...
        if (sounds[soundsIndex] != null)
            sounds[soundsIndex].setVolume(Game.getSettings().soundVolume);
    ...
    public static void playSong(@NotNull String songName, double volume, int fadeOutTicksLast, int fadeInTicks, final boolean loop) {
        ...
        if (currentMusic != null && currentMusic.getMusic().playing()) {
            ...
            currentMusic.setMusicListener(new MusicListener() {
                ...
                public void musicEnded(Music music) {
                    ...
                    currentMusic.fade(fadeIn, Game.getSettings().songVolume, false);
        ...
        currentMusic.setVolume(Game.getSettings().songVolume);

        if (!needWait) {
            currentMusic.start();
            currentMusic.fade(fadeInTicks, Game.getSettings().songVolume, false);
        }
    }
    ...
    private static String getSongPath(String name) {
        String real = Game.saver.makeResString(Game.saver.getRes() + SOUNDS + MUSIC + name + "." + SUFFIX_WAV);
        if (!Game.saver.existPath(real)) {
    ...
    private static String getSoundPath(String name) {
        String real = Game.saver.makeResString(Game.saver.getRes() + SOUNDS + SOUND + name + "." + SUFFIX_WAV);
        if (!Game.saver.existPath(real)) {
            real = SaverUtil.setSuffix(real, SUFFIX_OGG);
        }
        return real;
    ...
    public static void tick() {
        if (oldMusicVolume != Game.getSettings().songVolume) {
            oldMusicVolume = Game.getSettings().songVolume;
            if (currentMusic != null && currentMusic.getMusic().playing()) {
            ...
            if (ticksToStart[i] > 0) {
                ...
                if (ticksToStart[i] == 0) {
                    if (sounds[i] != null && !sounds[i].getSound().playing()) {
                        sounds[i].setVolume(Game.getSettings().soundVolume);
                        sounds[i].start();
                    }
    ...
    public static void setSongVolume(double songVolume, int fadeTicks) {
        if (currentMusic != null) {
            if (currentMusic.getMusic().playing()) {
                currentMusic.fade(fadeTicks, songVolume * Game.getSettings().songVolume, false);
            }
        }
    }
}
