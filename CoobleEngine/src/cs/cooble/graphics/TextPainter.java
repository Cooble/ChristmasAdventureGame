...
public class TextPainter implements MultiBitmapProvider {
    ...
    public void writeOnBitmap(@Nullable String[] rows) {
        ...
        g.clear();
        g.setColor(Game.renderer.isShadowHandled()?whiteBlank:blackBlank);
        g.fillRect(Game.getWorld().inventory().isOpen() ? 20*2 : 0,bitmap[0].getHeight() - 7 - rows.length * 9,bitmap[0].getWidth(),bitmap[0].getHeight());
        g.setColor(!Game.renderer.isShadowHandled()?Color.white:Color.black);
        g.setFont(font);
        for (int i = 0; i < rows.length; i++) {
            if (rows[i] == null)
                break;
            g.drawString(FontUtil.translate(rows[i]),Game.getWorld().inventory().isOpen() ? 22*2 : 2*2, bitmap[0].getHeight() - 15 - i * 9);
        }
        g.flush();
    }
